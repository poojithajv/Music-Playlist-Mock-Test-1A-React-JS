...
class Home extends Component {
  ...
  onDeleteItem = id => {
    ...
    const filterResult = playlist.filter(each => each.id !== id)
    this.setState({playlist: filterResult})
  }
  ...
  renderPlayList = () => {
    ...
    return SearchResult.length > 0 ? (
      <ul className="playlist-ul-container">
        {SearchResult.map(each => (
          <PlaylistItem
            key={each.id}
            playlistDetails={each}
            onDeleteSong={this.onDeleteItem}
          />
        ))}
      </ul>
    ) : (
      <div className="no-songs-view-container">
